...
import { LANGUAGES, COLOR_TESTS, TEST_MESSAGES, VISION_THRESHOLDS } from './constants';
...
function EyeSightTest() {
  ...
  const handleColorTest = (selectedColor: string, correctAnswer: string): void => {
    ...
    if (colorTestResults.length + 1 >= COLOR_TESTS.length) {
      const correctCount = colorTestResults.filter(result => result).length + (isCorrect ? 1 : 0);
  ...
  const handleVisualTest = (lineIndex: number): void => {
    const percentage = ((lineIndex + 1) / LANGUAGES[currentLanguage].chart.length) * 100;
    setTestResults({
      type: 'visual',
      score: percentage,
      details: `Read up to line ${lineIndex + 1} (${LANGUAGES[currentLanguage].chart[lineIndex].size}px font)`
    });
  };
  ...
  const handleReadAll = (): void => {
    setTestResults({
      type: 'visual',
      score: 100,
      details: `Read all lines perfectly (${LANGUAGES[currentLanguage].chart[LANGUAGES[currentLanguage].chart.length - 1].size}px font)`
    });
  };
  ...
  const printResults = (): void => {
    ...
    printWindow.document.write(`
      ...
      <body>
        ...
        <div class="results">
          <div class="score">
            ${testResults.type === 'visual' ? '👁️ Visual Acuity Test' : '🌈 Color Vision Test'}
          </div>
          <div class="details">
  ...
  const renderColorTest = (): JSX.Element | null => {
    ...
    return (
      <div>
        ...
        <div style={{ textAlign: 'center', marginBottom: '20px', fontSize: '3rem', fontWeight: 'bold' }}>
          Can you see the number: <span style={{ color: currentColorTest.correctAnswer }}>{currentColorTest.hiddenNumber}</span>
        </div>
        <div className={styles.colorTest}>
          {currentColorTest.colors.map((color, index) => (
            <div
              ...
              onClick={() => handleColorTest(color, currentColorTest.correctAnswer)}
            >
              {color === currentColorTest.correctAnswer ? currentColorTest.hiddenNumber : '?'}
            </div>
          ))}
        </div>
  ...
  return (
    <div className={styles.container}>
      ...
      <Navigation currentPath="/health-test/vision-tests/eye-sight" />
      <div className={styles.testCard}>
        <h1 className={styles.title}>👁️ Eye Sight Test</h1>
        <p className={styles.subtitle}>Professional vision screening with multiple languages and color vision testing</p>

        <div className={styles.testSelector}>
          <button 
            className={`${styles.testButton} ${currentTest === 'visual' ? styles.active : ''}`}
            onClick={() => { setCurrentTest('visual'); resetTest(); }}
          >
            📖 Visual Acuity Test
          </button>
          <button 
            className={`${styles.testButton} ${currentTest === 'color' ? styles.active : ''}`}
            onClick={() => { setCurrentTest('color'); resetTest(); }}
          >
            🌈 Color Vision Test
          </button>
        </div>
        ...
        <div className={styles.distanceInfo}>
          📏 Please sit 6 feet (2 meters) away from your screen for accurate results
        </div>
          ...
          <>
            <div className={styles.languageSelector}>
              {Object.entries(LANGUAGES).map(([key, lang]) => (
                <button
                  key={key}
                  className={`${styles.languageButton} ${currentLanguage === key ? styles.active : ''}`}
            ...
            <div className={styles.instructions}>
              <div className={styles.instructionTitle}>Visual Acuity Test Instructions:</div>
              <div className={styles.instructionText}>
                1. Cover one eye with your hand<br/>
                2. Read the letters from top to bottom<br/>
                3. Click "Can't read smaller" when letters become unclear<br/>
        ...
        {currentTest === 'color' && (
          <>
            <div className={styles.instructions}>
              <div className={styles.instructionTitle}>Color Vision Test Instructions:</div>
              <div className={styles.instructionText}>
                1. Look at each color pattern<br/>
                2. Try to identify the hidden number<br/>
                3. Click on the color that matches the question<br/>
                4. Complete all tests for accurate results
              </div>
            </div>

            <div className={styles.testArea}>
              {colorTestResults.length < COLOR_TESTS.length ? renderColorTest() : (
        ...
        {testResults && (
          <div className={styles.results}>
            <div className={styles.resultTitle}>
              {testResults.type === 'visual' ? '👁️ Visual Acuity Results' : '🌈 Color Vision Results'}
            </div>
        ...
        <div className={styles.instructions}>
          <div className={styles.instructionTitle}>⚠️ Important Disclaimer</div>
          <div className={styles.instructionText}>
            This is a basic vision screening tool and should not replace professional eye examinations. 
            For accurate diagnosis and treatment, please consult a qualified eye care professional.
          </div>
        </div>
      </div>
    </div>
